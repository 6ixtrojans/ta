...
/* Mod based on the work of leo7044 (https://github.com/leo7044) */
(function () {
    var OnlyStatsMain = function ()
    {
        function OnlyStatsCreate()
        {
            function setButtons()
            {
                ...
                qx.Class.define('OnlyStats',
                {
                    ...
                    members:
                    {
                        ...
                        {
                            // GUI-Fenster
                            this.GuiFenster = new qx.ui.window.Window('OnlyStats ' + scriptVersionLocal).set({
                                padding: 5,
                                paddingRight: 0,
                                width: 350,
                                showMaximize:false,
                            ...
                            // Tab 1: Main Switch
                            this.GuiInfoPage = new qx.ui.tabview.Page("Main Switch");
                            ...
                            // Tab 3: Player
                            this.GuiPlayerPage = new qx.ui.tabview.Page("Player Stats");
                        ...
                        showGui: function()
                        {
                            ...
                            HeadLineBases.add(new qx.ui.basic.Label('<big><b>If I Sell My Biggest Army, When Will I Have Enough Power to Rebuild It in Another Base</b></big>').set({rich: true}));
                            ...
                            var TextBaseName = new qx.ui.container.Composite(new qx.ui.layout.VBox(1).set({alignX: "left"}));
                            ...
                            var TextGoalPower = new qx.ui.container.Composite(new qx.ui.layout.VBox(1).set({alignX: "center"}));
                            var TextDaysToPower = new qx.ui.container.Composite(new qx.ui.layout.VBox(1).set({alignX: "center"}));
                            TextBaseName.add(new qx.ui.basic.Label('<b>Base</b>').set({rich: true}));
                            TextPower.add(new qx.ui.basic.Label('<b>Power/Hour</b>').set({rich: true}));
                            TextOwnPower.add(new qx.ui.basic.Label('<b>Available Power</b>').set({rich: true}));
                            TextGoalPower.add(new qx.ui.basic.Label('<b>Power Needed</b>').set({rich: true}));
                            TextDaysToPower.add(new qx.ui.basic.Label('<b>Days to Goal</b>').set({rich: true}));
                            var armyPowerGoal = 0;
                            var daysToGoal = 0;
                            for (var i in this.ObjectData.bases)
                            {
                              var sellPower = this.calculateValueOfOffense(this.ObjectData.bases[i].BaseId)[1];
                              if (sellPower > armyPowerGoal) {armyPowerGoal = sellPower;}
                            }
                            i = 0;
                            for (i in this.ObjectData.bases)
                            {
                              var valuePower = parseInt(ClientLib.Data.MainData.GetInstance().get_Cities().get_AllCities().d[this.ObjectData.bases[i].BaseId].GetResourceCount(ClientLib.Base.EResourceType.Power));
                                daysToGoal = ((armyPowerGoal - valuePower) / (this.ObjectData.bases[i].PowerPerHour)) / 24;
                                ...
                                TextGoalPower.add(new qx.ui.basic.Label(armyPowerGoal.toLocaleString()).set({rich: true, alignX: "right"}));
                                TextDaysToPower.add(new qx.ui.basic.Label(daysToGoal.toLocaleString()).set({rich: true, alignX: "right"}));
                            }
                            TableBases.add(TextBaseName);
                            TableBases.add(TextPower);
                            TableBases.add(TextOwnPower);
                            TableBases.add(TextGoalPower);
                            TableBases.add(TextDaysToPower);
                            HeadLineBases.add(TableBases);
                            HeaderTableBases.add(HeadLineBases);
                            this.GuiInfoVBox.add(HeaderTableBases);
                        ...
                        createTabPlayer: function()
                        {
                            ...
                            HeadLinePlayer.add(new qx.ui.basic.Label('<big><u><b>Player Stats</b></u></big>').set({rich: true}));
                       ...
                       getCurrentStats: function()
                        ...
                        getPublicAllianceInfo: function(_context, _data)
                        {
                            ...
                            this.ObjectData.alliance.Members = _data.m;
                        }

                    }
                });
                OnlyStats.getInstance().initialize();
    ...
    function Inject()
    {
        ...
        Script.textContent = "(" + OnlyStatsMain.toString() + ")();";
